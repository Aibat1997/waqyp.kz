 ...
 */
class ReportCategory extends Section implements Initializable
{
	public function initialize()
	{
		// Добавление пункта меню и счетчика кол-ва записей в разделе
		$this->addToNavigation($priority = 5, function () {
			return \App\ReportCategory::count();
		});
	}
	 ...
	 * @var string
	 */
	protected $title = 'Отчет (категория)';
	
	/**
	 * @var string
	 */
	protected $alias = 'report-category';
	...
	public function onEdit($id)
	{
		...
		$form = AdminForm::panel()
			->addHeader([
				$tabs,
				AdminFormElement::select('in_project_id', 'Подпроект')->setModelForOptions('App\InProject')
					->setLoadOptionsQueryPreparer(function ($element, $query) {
						return $query
							->where('status', 1)
							->where('is_report', 1);
					})->setDisplay('title_ru')->setSortable('title_ru')->required(),
