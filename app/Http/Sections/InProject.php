 ...
 */
class InProject extends Section implements Initializable
{
	public function initialize()
	{
		// Добавление пункта меню и счетчика кол-ва записей в разделе
		$this->addToNavigation($priority = 5, function () {
			return \App\InProject::count();
		});
	}
	 ...
	 * @var string
	 */
	protected $title = 'Подпроекты';
	
	/**
	 * @var string
	 */
	protected $alias = 'in-project';
		...
		$form = AdminForm::panel()
			->addHeader([
				...
				$columns = AdminFormElement::columns([
					[AdminFormElement::number('sort', 'Сортировка')->setDefaultValue(0)], [AdminFormElement::datetime('created_at', 'Дата и время')->setCurrentDate()],
				]),
				AdminFormElement::checkbox('status', 'Статус')->setDefaultValue(1),
				AdminFormElement::checkbox('is_report', 'Отчет')->setDefaultValue(0),
				AdminFormElement::select('project_id', 'Проект')->setModelForOptions('App\Project')->setDisplay('title_ru')->setSortable('title_ru')->required(),
